...
export const Permission = z
  .string()
  .regex(/^[a-z]+:[a-z\*]+$/, "Permission must be in format 'service:action'");
...
import {
  ...
  RemoveUserOutput,
  RemoveUserSchema,
} from "@/data/organizations/schemas";
...
export async function switchUserOrg(
  newOrgId: string,
...
export async function backfillOrgMemberships(
  ...
  for (const orgDoc of orgsSnapshot.docs) {
    ...
    const adminCount = memberships.docs.filter(
      (doc) =>
        (doc.data() as OrgMembership).baseRole === "admin" ||
        (doc.data() as OrgMembership).baseRole === "owner",
    ).length;
...
export async function syncRTDBFromFirestore(orgId: string): Promise<void> {
  ...
  for (const [userId, member] of Object.entries(membersMap)) {
    const effectivePerms = await calculateEffectivePermissions(
      userId,
      orgId,
      rolesMap as any,
    );
    await db
      .ref(`orgs/${orgId}/memberPermissions/${userId}`)
      .set(effectivePerms);
  }
}
```
...
describe("RemoveUserFromOrgUseCase", () => {
  ...
  beforeEach(() => {
    ...
    useCase = new RemoveUserFromOrgUseCase(
      membershipRepo,
      auditEmitter /* ... */,
    );
  });
...
describe("User Removal Flow (E2E Integration)", () => {
  ...
  it("should complete full removal & recovery flow", async () => {
    // 1. Remove user
    const removeResult = await removeUserFromOrg(
      "org1",
      "user1",
      "role change",
    );
...
describe("Security & RBAC", () => {
  ...
  it("should deny removal of last admin", async () => {
    ...
    // Attempt to remove
    await expect(removeUserFromOrg("org1", lastAdminUid)).rejects.toThrow(
      "Cannot remove last admin",
    );
  });
